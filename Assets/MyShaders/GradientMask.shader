...
////// Emissive:
                ...
                float3 emissive = lerp(tex2D(_yellow_asset,TRANSFORM_TEX(node_1168.rg, _yellow_asset)).rgb,tex2D(_organe_asset,TRANSFORM_TEX(node_1168.rg, _organe_asset)).rgb,_Blend);
                float3 finalColor = emissive;
/// Final Color:
                return fixed4(finalColor,lerp(tex2D(_invisible_shader,TRANSFORM_TEX(node_1168.rg, _invisible_shader)).a,tex2D(_rounded,TRANSFORM_TEX(node_1168.rg, _rounded)).a,_alpha_blend));
            }
            ENDCG
        }
    }
