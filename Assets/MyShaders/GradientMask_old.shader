...
/////// Diffuse:
                ...
                float2 node_1229 = i.uv0;
                finalColor += diffuseLight * lerp(tex2D(_yellow_asset,TRANSFORM_TEX(node_1229.rg, _yellow_asset)).rgb,tex2D(_organe_asset,TRANSFORM_TEX(node_1229.rg, _organe_asset)).rgb,_Blend);
/// Final Color:
                return fixed4(finalColor,lerp(tex2D(_invisible_shader,TRANSFORM_TEX(node_1229.rg, _invisible_shader)).a,tex2D(_rounded,TRANSFORM_TEX(node_1229.rg, _rounded)).a,_alpha_blend));
            }
            ENDCG
        }
        Pass {
...
/////// Diffuse:
                ...
                float3 finalColor = 0;
                float3 diffuseLight = diffuse;
                float2 node_1230 = i.uv0;
                finalColor += diffuseLight * lerp(tex2D(_yellow_asset,TRANSFORM_TEX(node_1230.rg, _yellow_asset)).rgb,tex2D(_organe_asset,TRANSFORM_TEX(node_1230.rg, _organe_asset)).rgb,_Blend);
/// Final Color:
                return fixed4(finalColor * lerp(tex2D(_invisible_shader,TRANSFORM_TEX(node_1230.rg, _invisible_shader)).a,tex2D(_rounded,TRANSFORM_TEX(node_1230.rg, _rounded)).a,_alpha_blend),0);
            }
            ENDCG
        }
    }
